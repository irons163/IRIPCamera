...
{
//    NSLog(@"CH[%d] addFrameIntoBuffer=%u" ,m_Channel,[m_aryFrameBuffer count]);
//    @synchronized(self)
//    {
//        NSLog(@"before clear buffer drop P=%u",[m_aryFrameBuffer count]);
    pthread_mutex_lock(&mtx);
        if((videoFrame.m_intFrameType == VIDEO_I_FRAME && [m_aryFrameBuffer count] > 0 ) || [m_aryDecodeBuffer count] >30)
        {
//            NSLog(@"CH[%d] clear buffer drop P=%u" ,m_Channel,[m_aryFrameBuffer count]);
            [self clearBuffer];
        }
...
-(void) clearBuffer
{
    //    NSLog(@"drop p=%d",[m_aryFrameBuffer count]);
